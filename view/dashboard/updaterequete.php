...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Créer un objet Requete et définir ses propriétés
    $requete = new Requete(
        $id_requete,
        $date,
        $type_de_requete,
    ...
    try {
        ...
        // Envoi d'un e-mail si la requête est marquée comme "traitée"
        if ($statut === "Traitée") {
            $to = "karchoudrania3@gmail.com";
            $subject = "Votre requête a été traitée";
            $message = "Bonjour,\n\nNous vous informons que votre requête (ID : $id_requete) a été traitée.\n\nMerci de votre confiance.";
            $headers = "rimajlassi702@gmail.com";
...
<div class="container">
    ...
    <form action="updaterequete.php?id_requete=<?php echo $requete['id_requete']; ?>" method="POST">
        ...
        <select id="statut" name="statut" required>
            <option value="Non traitée" <?php echo (isset($requete['statut']) && $requete['statut'] === "Non traitée") ? "selected" : ""; ?>>Non traitée</option>
            <option value="Traitée" <?php echo (isset($requete['statut']) && $requete['statut'] === "Traitée") ? "selected" : ""; ?>>Traitée</option>
        </select>
