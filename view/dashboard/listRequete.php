<?php
//Inclure le fichier contenant la classe RequeteController (ou RecommandationController si vous le souhaitez)
...
// Création de l'objet RequeteController (ou RecommandationController si vous le souhaitez)
$requeteC = new RequeteController();

// Vérifiez que la méthode listRequetes() existe dans la classe RequeteController
if (method_exists($requeteC, 'listRequetes')) {
    // Récupération de la liste des requêtes
    $listeRequetes = $requeteC->listRequetes();
} else {
    die('Erreur : La méthode listRequetes() n\'existe pas dans la classe RequeteController.');
}
?>
<!DOCTYPE html>
<html lang="fr">
                ...
                <div class="table-wrapper">
                    <div class="table-container">
                        ...
                        <table>
                            ...
                            <?php if (isset($listeRequetes)) {
                                foreach ($listeRequetes as $requete) { ?>
                                <tr>
                                    <td><?= htmlspecialchars($requete['id_requete']); ?></td>
                                    <td><?= htmlspecialchars($requete['date']); ?></td>
                                    <td><?= htmlspecialchars($requete['type_de_requete']); ?></td>
                                    <td class="actions">
                                        <a href="updaterequete.php?id_requete=<?= htmlspecialchars($requete['id_requete']); ?>">Modifier</a>
                                        <a href="deleterequete.php?id_requete=<?= urlencode($requete['id_requete']); ?>">Supprimer</a>
                                    </td>
                                </tr>
                            <?php }
                            } ?>
